...
Follow the format defined in `/autonomous-decisions`. Document every non-trivial decision. Collect decisions into the PR body's "Autonomous Decisions" section.
...
- Pre-landing review ASK items (decide autonomously, document reasoning)
- MINOR or MAJOR version bump needed (decide autonomously based on diff analysis)
- AI-assessed coverage below minimum threshold (generate more tests automatically, document if overriding)
- Plan items NOT DONE (decide autonomously: ship with TODOs if minor, stop if core functionality missing)
- Plan verification failures (fix if functional, ship if cosmetic, document reasoning)
- TODOS.md missing (create it automatically)
- TODOS.md disorganized (reorganize it automatically)
...
4. Check diff size: `git diff <base>...HEAD --stat | tail -1`. If the diff is >200 lines, note: "Large diff — consider running a thorough review before shipping."
...
3. **If no release pipeline exists and a new artifact was added:** Decide autonomously:
   - If the artifact is clearly internal/web-only and existing deployment covers it, proceed silently. Document: "AUTONOMOUS DECISION: No release pipeline needed — artifact is internal/web-only."
   - If the artifact is a distributable binary/tool that users need to download, add the release workflow now. Document: "AUTONOMOUS DECISION: Adding release workflow — new distributable artifact detected."
   - If unclear, defer to TODOS.md with a P1 entry: "Add release pipeline for {artifact}". Document: "AUTONOMOUS DECISION: Deferred release pipeline to TODOS.md — unclear distribution needs."
...
**If NO runtime detected** (no config files found): Write `.no-test-bootstrap` and continue without tests. Document: "AUTONOMOUS DECISION: No runtime detected — skipping test bootstrap."
...
Autonomously select the primary recommendation from the table above for the detected runtime. Document: "AUTONOMOUS DECISION: Selected {framework} for {runtime} — primary recommendation per best practices table. Includes: {packages}. Supports: unit, integration, smoke, e2e."

If multiple runtimes detected (monorepo), set up the primary runtime first, then the secondary sequentially.
...
Determine repo ownership mode:
- If you are the sole contributor (solo project), investigate and fix pre-existing test failures. Document: "AUTONOMOUS DECISION: Fixing pre-existing test failures in solo repo — we own everything."
  - Switch to /investigate mindset: root cause first, then minimal fix.
  - Fix the pre-existing failure.
  - Commit the fix separately: `git commit -m "fix: pre-existing test failure in <test-file>"`
  - Continue with the workflow.
  - If the fix is too complex (would take more than ~15 minutes of agent time), add a P0 TODO instead and continue:
    - Document: "AUTONOMOUS DECISION: Pre-existing test failure too complex to fix inline — added P0 TODO."
    - If `TODOS.md` exists, add the entry.
    - If `TODOS.md` does not exist, create it with the standard header and add the entry.
    - Continue with the workflow — treat the pre-existing failure as non-blocking.

- If it is a collaborative repo, create a GitHub/GitLab issue assigned to the likely author:
  ...
  - Document: "AUTONOMOUS DECISION: Created issue for pre-existing test failure — assigned to {author} (collaborative repo)."
  - Continue with the workflow.
...
- **>= minimum, < target:** Autonomously generate more tests for remaining gaps (one additional pass). Document: "AUTONOMOUS DECISION: Coverage at {X}% (below target {target}%) — generating additional tests." After second pass, if still below target, accept and document in PR body: "Coverage gate: {X}% — additional test generation pass completed, remaining gaps documented in diagram."
- **< minimum:** Autonomously generate tests for remaining gaps (up to 2 passes). Document: "AUTONOMOUS DECISION: Coverage critically low at {X}% (below minimum {minimum}%) — generating tests." If still below minimum after 2 passes, accept and document in PR body: "Coverage gate: {X}% after 2 generation passes — remaining gaps require manual attention." Include the coverage diagram.
...
- **Any NOT DONE items:** Decide autonomously based on severity:
  - **If 1-2 minor items (docs, config, non-core):** Ship anyway and create P1 TODOs in Step 5.5. Document: "AUTONOMOUS DECISION: Shipping with {N} minor NOT DONE items deferred to TODOs — {list items}."
  - **If core functionality is missing (>2 items or any critical feature):** STOP. List the missing items. Document: "AUTONOMOUS DECISION: Stopping — core functionality missing from plan: {list items}."
  - **If items were clearly superseded by implementation changes:** Treat as intentionally dropped. Document: "AUTONOMOUS DECISION: Plan items dropped — superseded by implementation: {list items}."
...
- **Any FAIL:** Decide autonomously based on severity:
  - **Functional failures (broken features, missing pages, errors):** Fix the failures before shipping. Document: "AUTONOMOUS DECISION: Fixing functional verification failures before shipping."
  - **Cosmetic failures (spacing, alignment, minor visual issues):** Ship anyway with known issues documented. Document: "AUTONOMOUS DECISION: Shipping with cosmetic verification failures — {list issues}."
...
```bash
git diff origin/<base> --name-only | grep -E '\.(tsx?|jsx?|css|scss|less|html|vue|svelte)$' | head -5
```

**If no frontend files found:** Skip design review silently. No output.
...
4. **Apply the design checklist** against the changed files. For each item:
   ...
   - **[HIGH/MEDIUM] design judgment needed**: classify as AUTO-FIX and apply best-judgment fix. Document: "AUTONOMOUS DECISION: Applied design fix for {issue} — {reasoning}."
   - **[LOW] intent-based detection**: note as informational in the review output: "Possible — verify visually or run /design-review"
...
4. **Classify each finding as AUTO-FIX or ASK** per the Fix-First Heuristic in
   checklist.md. In autonomous mode, treat all findings as AUTO-FIX where a clear fix exists. For findings that require judgment:
   - Apply the fix that best aligns with the project's existing patterns and conventions.
   - Document: "AUTONOMOUS DECISION: Fixed {issue} — {reasoning for chosen approach}."
   - If truly ambiguous (multiple valid approaches with different tradeoffs), choose the more conservative/safe option and document the alternatives considered.

5. **Auto-fix all fixable items.** Apply each fix. Output one line per fix:
   `[AUTO-FIXED] [file:line] Problem → what you did`

6. **For findings where no clear fix exists** (e.g., architectural concerns, design tradeoffs), note them as informational in the PR body but do not block shipping.

7. **After all fixes:**
   - If ANY fixes were applied: commit fixed files by name (`git add <fixed-files> && git commit -m "fix: pre-landing review fixes"`), then re-run tests to verify fixes don't break anything. If tests pass, continue. If tests fail, revert the fix and note in PR body.
   - If no fixes applied (no issues found): continue to Step 4.

8. Output summary: `Pre-Landing Review: N issues — M auto-fixed, K informational (noted in PR body)`
...
For each classified comment, decide autonomously:

**VALID & ACTIONABLE:** Apply the fix. Commit: `git add <fixed-files> && git commit -m "fix: address Greptile review — <brief description>"`. Reply using the **Fix reply template** from greptile-triage.md (include inline diff + explanation). Document: "AUTONOMOUS DECISION: Fixed Greptile finding — {description}."

**VALID BUT ALREADY FIXED:** Reply using the **Already Fixed reply template** from greptile-triage.md — include what was done and the fixing commit SHA.

**FALSE POSITIVE:** Reply using the **False Positive reply template** from greptile-triage.md (include evidence + suggested re-rank). Document: "AUTONOMOUS DECISION: Marked as false positive — {evidence}."

**For findings requiring complex judgment (unclear if valid, or fix has significant tradeoffs):**
- If the fix is low-risk and aligns with project conventions, apply it. Document reasoning.
- If the fix is high-risk or architecturally significant, note it as informational in the PR body without fixing. Document: "AUTONOMOUS DECISION: Noted Greptile finding as informational — {reasoning for not fixing}."
...
Present findings under an `ADVERSARIAL REVIEW (Claude subagent):` header. **FIXABLE findings** flow into the same Fix-First pipeline as the structured review. **INVESTIGATE findings** are presented as informational in the PR body.
...
If GATE is FAIL: Autonomously investigate and fix the critical issues. Document: "AUTONOMOUS DECISION: Fixing {N} critical Codex findings — {brief descriptions}." After fixing, re-run tests (Step 3) since code has changed. Re-run `codex review` to verify.
...
If Codex is not available for steps 1 and 3, note: "Codex CLI not found — large-diff review ran Claude structured + Claude adversarial (2 of 4 passes). Install Codex for full 4-pass coverage: `npm install -g @openai/codex`"
...
2. **Auto-decide the bump level based on the diff:**
   ...
   - **MINOR** (2nd digit): Major features or significant architectural changes. Decide autonomously. Document: "AUTONOMOUS DECISION: MINOR bump — {reasoning based on diff analysis}."
   - **MAJOR** (1st digit): Milestones or breaking changes. Decide autonomously. Document: "AUTONOMOUS DECISION: MAJOR bump — {reasoning based on diff analysis}."
...
Cross-reference the project's TODOS.md against the changes being shipped. Mark completed items automatically.

Read `.claude/skills/review/TODOS-format.md` for the canonical format reference.

**1. Check if TODOS.md exists** in the repository root.

**If TODOS.md does not exist:** Create it automatically with a skeleton (# TODOS heading + ## Completed section). Document: "AUTONOMOUS DECISION: Created TODOS.md — recommended for tracking project work." Continue to step 3.
...
**If disorganized** (missing priority fields, no component groupings, no Completed section): Reorganize automatically in-place following TODOS-format.md. Preserve all content — only restructure, never delete items. Document: "AUTONOMOUS DECISION: Reorganized TODOS.md to follow recommended structure."
...
<List all AUTONOMOUS DECISION entries from this run, with reasoning>
...
- **Never ask for trivial confirmations** (e.g., "ready to push?", "create PR?"). Decide autonomously for all decision points, documenting reasoning.
...
- **The goal is: `/ship` runs, next output is the review + PR URL + auto-synced docs. No human interaction needed.**
- **Document all autonomous decisions.** Every decision point that would have asked the user gets an "AUTONOMOUS DECISION:" entry with reasoning. These are collected into the PR body's "Autonomous Decisions" section.
