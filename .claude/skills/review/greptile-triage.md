...
PROJECT_HISTORY="$HOME/.agent/projects/$REMOTE_SLUG/greptile-history.md"
```
...
Use Tier 2 when escalation detection (below) identifies a prior prior automated reply on the same thread. Include maximum evidence to close the discussion.
...
Before composing a reply, check if a prior prior automated reply already exists on this comment thread:

1. **For line-level comments:** Fetch replies via `gh api repos/$REPO/pulls/$PR_NUMBER/comments/$COMMENT_ID/replies`. Check if any reply body contains review markers: `**Fixed**`, `**Not a bug.**`, `**Already fixed**`.

2. **For top-level comments:** Scan the fetched issue comments for replies posted after the Greptile comment that contain review markers.

3. **If a prior prior automated reply exists AND Greptile posted again on the same file+category:** Use Tier 2 (firm) templates.

4. **If no prior prior automated reply exists:** Use Tier 1 (friendly) templates.
...
```bash
REMOTE_SLUG=$(basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
mkdir -p "$HOME/.agent/projects/$REMOTE_SLUG"
mkdir -p ~/.agent
```

Append one line per triage outcome to **both** files (per-project for suppressions, global for retro):
- `~/.agent/projects/$REMOTE_SLUG/greptile-history.md` (per-project)
- `~/.agent/greptile-history.md` (global aggregate)
