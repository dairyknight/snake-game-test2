...
Check if the diff touches frontend files. Examine the diff stat to determine if frontend files (HTML, CSS, JSX, TSX, Vue, Svelte, etc.) are changed:

```bash
git diff origin/<base> --stat | grep -E '\.(html|css|scss|less|jsx|tsx|vue|svelte|astro)' && echo "SCOPE_FRONTEND=true" || echo "SCOPE_FRONTEND=false"
```
...
4. **Apply the design checklist** against the changed files. For each item:
   ...
   - **[HIGH/MEDIUM] design judgment needed**: classify as AUTONOMOUS-DECISION — the agent makes the call and documents its reasoning
...
Include any design findings alongside the findings from Step 4. They follow the same Fix-First flow in Step 5 — AUTO-FIX for mechanical CSS fixes, autonomous decision for everything else.
...
- Classify each gap per the Fix-First Heuristic:
  - **AUTO-FIX:** Simple unit tests for pure functions, edge cases of existing tested functions — generate, run, and commit immediately
  - **AUTONOMOUS-DECISION:** E2E tests, tests requiring new test infrastructure, tests for ambiguous behavior — the agent decides whether to write them based on risk assessment. Document the reasoning: "Writing this E2E test because [reason]" or "Skipping this E2E test because [reason — e.g., no E2E framework configured, low-risk path]"
- For AUTO-FIX gaps: generate the test, run it, commit as `test: coverage for {feature}`
- For paths marked [→E2E]: the agent decides autonomously whether to write them. Write them if an E2E framework is available and the path is high-risk. Skip with documented reasoning if not.
- For paths marked [→EVAL]: the agent decides autonomously whether to write them. Write them if eval infrastructure exists. Skip with documented reasoning if not.
...
For each finding, classify as AUTO-FIX or AUTONOMOUS-DECISION per the Fix-First Heuristic in
checklist.md. Critical findings lean toward AUTONOMOUS-DECISION (document reasoning); informational findings lean
toward AUTO-FIX.
...
### Step 5c: Autonomously decide on remaining items

For items that would normally require human input (ASK items in the original workflow), the agent makes the decision autonomously:

1. **For each item**, evaluate:
   - Severity and risk of the issue
   - Confidence in the recommended fix
   - Whether the fix could introduce new problems
   - Whether reasonable engineers would disagree about the approach

2. **Decision framework:**
   - **Fix if:** The issue is clearly a bug, the fix is well-understood, and the risk of NOT fixing outweighs the risk of the fix
   - **Fix if:** It's a security issue — always err on the side of fixing security problems
   - **Skip if:** The fix is speculative, could change user-visible behavior in unexpected ways, or requires domain knowledge the agent doesn't have
   - **Skip if:** The fix would be a large refactor (>50 lines) with unclear benefit

3. **Output format for autonomous decisions:**
```
AUTONOMOUS DECISIONS:
─────────────────────────────────
[FIXED — autonomous] [file:line] Problem → fix applied
  REASONING: [1 sentence explaining why this was fixed without human input]

[SKIPPED — autonomous] [file:line] Problem description
  REASONING: [1 sentence explaining why this was left for human review]
  RECOMMENDED ACTION: [what a human should do about it]
─────────────────────────────────
```

### Step 5d: Apply autonomous fixes

Apply fixes for all items where the agent decided to fix. Output what was fixed.
...
1. **VALID & ACTIONABLE comments:** These are included in your findings — they follow the Fix-First flow (auto-fixed if mechanical, autonomous decision if not). If the agent decides to fix, reply using the **Fix reply template** from greptile-triage.md (include inline diff + explanation). If the agent determines it's a false positive, reply using the **False Positive reply template** (include evidence + suggested re-rank), save to both per-project and global greptile-history.

2. **FALSE POSITIVE comments:** The agent autonomously decides how to handle each one:
   - If clearly wrong: reply to Greptile explaining why, using the **False Positive reply template** from greptile-triage.md (include evidence + suggested re-rank), save to both per-project and global greptile-history.
   - If low-effort and harmless to fix anyway: fix it and reply with the fix.
   - Document the reasoning for each decision.

3. **VALID BUT ALREADY FIXED comments:** Reply using the **Already Fixed reply template** from greptile-triage.md — no decision needed:
...
If GATE is FAIL, the agent autonomously investigates and fixes the critical issues found by Codex. Document each fix:
```
CODEX GATE FAIL — autonomous remediation:
  [FIXED] Issue description → fix applied
  [SKIPPED] Issue description → REASONING: [why this was not fixed]
```

Re-run `codex review` to verify after fixes.
...
- **Fix-first, not read-only.** AUTO-FIX items are applied directly. Items that would normally require human input are decided autonomously with documented reasoning. Never commit, push, or create PRs — that's /ship's job.
- **Be terse.** One line problem, one line fix. No preamble.
- **Only flag real problems.** Skip anything that's fine.
- **Use Greptile reply templates from greptile-triage.md.** Every reply includes evidence. Never post vague replies.
- **Document all autonomous decisions.** Every decision that would have been asked of a human must include a REASONING line explaining the choice.
