...
**Trigger:** Only run this checklist if the diff touches frontend files. Check the diff stat for frontend file extensions:

```bash
git diff <base>...HEAD --stat | grep -qE '\.(tsx?|jsx?|css|scss|html|vue|svelte)' && echo "SCOPE_FRONTEND=true" || echo "SCOPE_FRONTEND=false"
```
