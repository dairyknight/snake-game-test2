...
**Tier 2 — global skills (autonomous decision):** Autonomously scan globally installed skills and hooks for malicious patterns. This reads files outside the repo but is necessary for a complete security audit. Document that global skill scanning was performed.

Run the same Grep patterns on globally installed skill files at `~/.claude/skills/` and check hooks in user settings. If access is denied, note "Global skill scan: access denied — repo-local only" and proceed.
...
**FP rules:** Skills that use `curl` for legitimate purposes (downloading tools, health checks) need context — only flag when the target URL is suspicious or when the command includes credential variables.
...
**Trend Tracking:** If prior reports exist in `.agent/security-reports/`:
```
...
**Remediation Roadmap (autonomous):** For the top 5 findings, produce a structured remediation plan. For each finding, autonomously select the best remediation approach and document the reasoning:
...
Finding #N: [Title]
  Severity: [CRITICAL/HIGH/MEDIUM]
  Exploit scenario: [brief summary]
  Selected approach: [Fix now / Mitigate / Accept risk / Defer]
  Reasoning: [Why this approach was chosen]
  Action: [Specific code change or workaround with effort estimate]
  Review date: [If accepting risk, when to revisit]
```

Decision criteria for autonomous remediation selection:
- CRITICAL findings: Always select "Fix now" unless technically impossible
- HIGH findings: Select "Fix now" if effort is low-medium; "Mitigate" if fix requires major refactor
- MEDIUM findings: Select based on effort-to-risk ratio; "Defer" is acceptable with documented review date
- When in doubt, prefer the more secure option

### Phase 14: Save Report

```bash
mkdir -p .agent/security-reports
```

Write findings to `.agent/security-reports/{date}-{HHMMSS}.json` using this schema:
...
If `.agent/` is not in `.gitignore`, note it in findings — security reports should stay local.
